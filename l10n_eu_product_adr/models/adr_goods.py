...
class AdrGoods(models.Model):
    ...
    def name_get(self):
        ...
        for rec in self:
            ...
            if rec.limited_quantity or rec.limited_quantity_uom_id:
                affixes.append(
                    _("qty:{limited_quantity} {limited_quantity_uom_id}").format(
                        limited_quantity=rec.limited_quantity or 0.0,
                        limited_quantity_uom_id=rec.limited_quantity_uom_id.name or "",
                    )
                )
            if affixes:
                name += " (%s)" % (", ".join(affixes))
