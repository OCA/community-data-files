...
class NaceImport(models.TransientModel):
    ...
    delete_obsolete_records = fields.Boolean(default=False)
    ...
    def _download_nace(self, lang_code):
        ...
        content_fixed = re.sub(
            pattern, '<?xml', res_request.content.decode('utf-8'))
    ...
    def translate_nace(self, node, lang):
        ...
        if nace:
            for field, value in trans.items():
                ...
                if translation:
                    translation_model.with_context(lang=lang.code).write(data)
                    # TODO  2019-01-18: dangerous code, write in ir_translation
                    # self.env.cr.execute(
                    #     "UPDATE ir_translation SET value = "
                    #     "'%s' WHERE id = %s", (
                    #         value.replace("'", "''"), translation.id))
                else:
                    data['res_id'] = nace.id
                    data['type'] = 'model'
                    data['name'] = name
                    data['lang'] = lang.code
                    data['module'] = 'l10n_eu_nace'
    ...
    def run_import(self):
        ...
        langs = lang_model.search(
            [('code', 'in', list(self._available_langs.keys())),
             ('active', '=', True)])
        ...
        dom = etree.fromstring(xmlcontent.encode('utf-8'), parser=utf8_parser)
        for node in dom.iter('Item'):
            logger.debug(
                'Reading level=%s, code=%s', node.get('idLevel', 'N/A'),
                node.get('id', 'N/A'))
        ...
        for lang in langs:
            ...
            dom = etree.fromstring(
                xmlcontent.encode('utf-8'), parser=utf8_parser)
            for node in dom.iter('Item'):
                logger.debug(
                    'Reading lang=%s, level=%s, code=%s', nace_lang,
                    node.get('idLevel', 'N/A'), node.get('id', 'N/A'))
                self.translate_nace(node, lang)
        # Delete obsolete NACEs
        if self.delete_obsolete_records and naces_to_delete:
